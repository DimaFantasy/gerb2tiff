...
extern "C" __declspec(dllexport) int processGerber(const GerberOptions &options)
{
    ...
    if (options.optVerbose > 0) optQuiet = false; // if user wants verbose, then cancel quiet option

    if (options.imageDPI < 1) error("DPI setting must be >= 1");
    if (options.optBoarder < 0) error("boarder setting must be >= 0");
