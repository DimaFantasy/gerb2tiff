...
int main(int argc, char **argv)
{
	...
	std::cin.get(); // Добавьте эту строку здесь, после вывода аргументов
	return 0;
	...
	while (1)
	{
		...
		std::cout << "getopt_long returned: " << c << std::endl; // Добавьте эту строку здесь
		...
		case 2:
			...
			break;
		case 3:
			std::cout << "Обработка --help\n"; // Отладочный вывод
			std::cout << "Справка по программе:\n";
			// Вывод справки
			return 0;
			break;
		case 4:
			// Обработка --grow-pixels
			break;
		case 5:
			// Обработка --grow-mm
			break;
		case 6:
			// Обработка --strip-rows
			break;
		case 'b':
			// Обработка --boarder-mm
			break;
		case 7:
			// Обработка --boarder-pixels
			break;
		case 8:
			// Обработка --rotation
			break;
	...
	if (imageDPI < 1)
		error(string("DPI setting must be >= 1"));
	if (optBoarder < 0)
		error(string("boarder setting must be >= 0"));
